...
>
> **Status:** `--tool`, `--global/--local`, `--dry-run`, `--deploy`, `--verbose`, `--force`, `--yes` are live. The plan is always computed and rendered; `--deploy` then **writes it** (atomic, idempotent) and records state (§6c/§6d). Default and `--dry-run` write nothing. `--profile`, `--recipe`, `--prefer-system-pkg` arrive with the profile and recipe engines (Phases 4–5).
