...
If your stream names no spec or plan, STOP and tell the user: "This stream has no spec or plan. The canonical path is `/team-research` (research) → `spec-brainstorming` (spec) → `plan-writing` (plan); run those first before using `/plan-execute-parallel`."
