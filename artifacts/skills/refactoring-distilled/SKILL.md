---
name: refactoring-distilled
description: Distilled Refactoring decision and trigger rules (after Fowler's "Refactoring") — behavior-preserving change in small steps, code smells, safety nets, when to extract/rename/inline. LLM-derived and inspired by, NOT a reproduction of, the book. Use when changing existing code, cleaning up structure, or preparing a feature/bugfix in messy code.
---
...
- Pick the simplest solution that does not become an antipattern. When one option is smaller today but seeds a duplicate implementation, a parallel source of truth, or a special case that must be kept in sync, it is not the simplest — it is deferred cost. Prefer the option with a single source of truth even when it edits shared code, as long as the edit is behavior-preserving; reject both the isolated-but-duplicative shortcut and the speculative over-abstraction. The tie-breaker is: which choice leaves exactly one place that owns this fact?
